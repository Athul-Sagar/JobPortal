...
const JobListing = () => {
    ...
    return (
        <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
                    ...
                    <ul className='space-y-4 text-gray-600'>
                        {JobCategories.map((category, index) => (
                            <li key={index} className="flex items-center gap-2">
                                <input type="checkbox" id={`category-${index}`} className="w-4 h-4 scale-125" onChange={() => handleCategoryChange(category)} checked={selectedCategories.includes(category)} />
                ...
                <div className={showFilter ? "" : "max-lg:hidden"}>
                    ...
                    <ul className='space-y-4 text-gray-600'>
                        {JobLocations.map((location, index) => (
                            <li key={index} className="flex items-center gap-2">
                                <input type="checkbox" id={`location-${index}`} className="w-4 h-4 scale-125" onChange={() => handleLocationChange(location)} checked={selectedLocation.includes(location)} />
            ...
            <section className='w-full lg:w-3/4 text-gray-800 max-lg:px-4'>
                ...
                <p className='mb-8'>Get your Desired jon from the top companies</p>
                <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4  '>

                {filteredJobs.slice((currentPage - 1) * 6, currentPage * 6).map((job) => (
    <JobCard key={job.id} job={job} /> // Use job.id instead of array index
))}
    ...
    <a href="#job-list" key={index}> {/* Add key here */}
        <button 
