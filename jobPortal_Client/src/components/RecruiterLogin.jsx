...
const RecruiterLogin = ({ onClose }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      if (state === "Login") {
        const { data } = await axios.post(backendUrl + '/api/company/login',{ email, password }, { withCredentials: true });
        ...
        <form onSubmit={onSubmitHandler} className='relative bg-white p-10 rounded-xl text-slate-500'>
          ...
          {state === "Sign Up" && isTextDataSubmited ? (
            ...
            <div className="">
              ...
              <div className="border px-4 py-2 flex items-center gap-2 rounded-full mt-5">
                <img src={assets.lock_icon} alt="lock" className="" />
                <input
                  type="password"
          ...
          <button type='submit' className="bg-blue-600 w-full text-white py-2 rounded-full mt-4">
            {state === 'Login' ? 'Sign In' : isTextDataSubmited ? 'Create Account' : "Next"}
          </button>
